          ...
          Lwt_pool.use Db.pool (fun conn ->
              ...
              | Some (cfg, version, updated_at, updated_by) ->
                  Dream.json ~code:200 ~headers:json_headers
                    (Config.make_get_response_json ~config_json:cfg ~version
                       ~updated_at ~updated_by ) ) ) )
    (fun exn -> error_json ~code:500 (Printexc.to_string exn))

let putConfig request =
  Lwt.catch
    (fun () ->
      Helpers.checkPrems request (fun () ->
          Dream.body request
              ...
              Lwt_pool.use Db.pool (fun conn ->
                  ...
                  let rec attempt retries =
                    ...
                    if unchanged then
                      ...
                      let history_json =
                        Config.make_history_entry_json ~version:next_version
                          ~timestamp:ts ~changed_by:actor ~action:"update"
                      ...
                      | _ ->
                          Dream.json ~code:200 ~headers:json_headers
                            (Config.make_get_response_json
                               ~config_json:new_cfg_json
                               ~version:next_version ~updated_at:ts
                               ~updated_by:actor )
                  in
                  attempt 0 ) ) )
    (fun exn -> error_json ~code:500 (Printexc.to_string exn))

let getConfigHistory request =
  Lwt.catch
    (fun () ->
      Helpers.checkPrems request (fun () ->
          Lwt_pool.use Db.pool (fun conn ->
              ...
              in
              Dream.json ~code:200 ~headers:json_headers
                (Config.make_history_response_json (collect [] rows)) ) ) )
    (fun exn -> error_json ~code:500 (Printexc.to_string exn))
